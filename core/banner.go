...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - --  Private @dark_logger Edition For Education Purposes Only  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by Developer (") + handleClr.Sprintf("@dark_logger") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
